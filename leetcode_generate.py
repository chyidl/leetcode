...
#
# Created by Chyi Yaqing on 02/28/19 19:00.
...
HEADERS = {
    ...
    "Host": "leetcode.com",
    "User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36",  # NOQA
}
...
class QuizItem:
    ...
    def url(self):
        return "{base_url}/problems/{question__title_slug}".format(
            base_url=self.base_url, question__title_slug=self.question__title_slug
        )
...
class Leetcode:
    ...
    def login(self):
        ...
        if not CONFIG["username"] or not CONFIG["password"]:
            raise Exception(
                "Leetcode - Please input your username and password in config.ini."
            )
        usr = CONFIG["username"]
        pwd = CONFIG["password"]
        ...
        self.cookies = {
            str(cookie["name"]): str(cookie["value"]) for cookie in webdriver_cookies
        }
    ...
    def load_submissions(self):
        ...
        while True:
            ...
            submissions_url = (
                "{}/api/submissions/?format=json&limit={}&offset={}&last_key={}".format(
            ...
            if item.is_lock:
                ...
                if item.solutions:
                    dirname = "{id}-{title}".format(
                        id=str(item.question_id).zfill(4),
                        title=item.question__title_slug,
                    )
                    language = ""
                    language_lst = [
                        i["lang"] for i in item.solutions if i["lang"] in self.languages
                    ]
                    while language_lst:
